...
param (
    ...
    [Parameter(Mandatory=$false)] [string]$extra = "intel"
)
...
function Install-PythonDependencies {
    ...
    uv venv --clear --python $global:PYTHON_VERSION venv_release_win_arc
    .\venv_release_win_arc\Scripts\Activate.ps1
    ...
    uv pip install patch
    uv run --no-project python -m patch -p1 -d venv_release_win_arc/lib/site-packages patches/increase_mms_time_limit.patch
    uv run --no-project python -m patch -p1 -d venv_release_win_arc/lib/site-packages patches/remove_ultralytics_telemetry.patch
    uv run --no-project python -m patch -p1 -d venv_release_win_arc/lib/site-packages patches/fix_loading_mmengine_weights_on_torch26_and_higher.diff
    uv pip uninstall patch
...
function Create-EXE {
    ...
    uv run --no-project pyinstaller --noconfirm ./packaging/windows/lada-arc.spec -- $cliOnlyArg
